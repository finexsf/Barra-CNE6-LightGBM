...
'''
如果想要5年的训练数据，那就需要获取5年的股票数据前面再加5年，也就是一共10年的基本数据，才能计算出五年期因子，因为有的三级因子需要前5年的数据才能计算
但是大部分三级因子需要用过去1年的数据来算，即计算五年期因子需要6年的数据
'''

def read_data(delay):
    print('正在读取数据')
    print(f"使用因子数据: {os.path.abspath('result/全部三级因子.csv')}")  # 把传入的相对路径转换成绝对路径
    ...
    # 国家因于对任何股票的暴露值都是1，国家因子对应一个字段country
    barra_df['Country'] = 1
...
if __name__ == "__main__":
    ...
    # 训练窗口天数, 一年大概250个交易日
    # 设置好 train_window 训练窗口天数，就会按照比例把训练窗口分成训练集和验证集 val_start = train_df['time'].quantile(0.8)
    ...
    while end_index < len(all_dates)-1:
        ...
        if model_type == 'reg':
            ...
            # 回归模型评估
            rmse = mean_squared_error(test_data_y_reg, pred, squared=False)  # squared=False 计算均方根误差RMSE，squared=True 计算均方误差MSE    
