'''
这份代码和lightgbm_train.py的区别：
只用barra cne6因子训练，去掉行业因子和国家因子
因为我发现因子特征重要性排序里，前面的基本都是行业因子，这样就把真正要分析的barra cne6因子淹没掉了
'''
...
def read_data(delay):
    print('正在读取数据...')
    print(f"使用因子数据: {os.path.abspath('/home/xusifan/LL/cal_cne6_902/tushare-20120101_20250101/result/全部三级因子.csv')}")
    barra_df = read_df('/home/xusifan/LL/cal_cne6_902/tushare-20120101_20250101/result/全部三级因子.csv')
    ...
    all_dates = sorted(pd.to_datetime(read_df('/home/xusifan/LL/cal_cne6_902/tushare-20120101_20250101/可用交易日_SZSE.csv')['cal_date'], format='%Y%m%d').to_list())
    ...
    print(f'获取股票价格数据的起始日期 = start_date: {start_date.strftime("%Y-%m-%d")}')
    # 获取因子数据的最晚日期，并向后延长 delay+30 天，作为结束日期
    end_date = barra_df['time'].max() + pd.Timedelta(days=delay + 30)
    print(f'获取股票价格数据的结束日期: {barra_df["time"].max().strftime("%Y-%m-%d")}')
    print(f'将股票价格数据的结束日期延长30个交易日 = end_date: {end_date.strftime("%Y-%m-%d")}')
...
def estimate(model, prefix=''):
    """绘制LightGBM特征重要性条形图（gain/split）"""
    # (a) gain
    plt.figure(figsize=(12, 6))
    ax1 = lgb.plot_importance(
        model,
        importance_type="gain",
        max_num_features=30,
        figsize=(12, 6)
    )
    ax1.set_title("")  # 清空原有标题
    ax1.set_xlabel("Feature Importance", fontsize=15)
    ax1.set_ylabel("Features", fontsize=15)
    ax1.tick_params(axis='x', labelsize=12)
    ax1.tick_params(axis='y', labelsize=12)
    plt.tight_layout()
    plt.savefig(f'/home/xusifan/LL/cal_cne6_902/photo_train/{prefix}feature_importance_gain.png', dpi=300)
    plt.close()
    print(f'特征重要性（gain）图已保存到 /home/xusifan/LL/cal_cne6_902/photo_train/{prefix}feature_importance_gain.png')
    ...
    ax2.set_title("")  # 清空原有标题
    ax2.set_xlabel("Feature Importance", fontsize=15)
    ax2.set_ylabel("Features", fontsize=15)
    ax2.tick_params(axis='x', labelsize=12)
    ax2.tick_params(axis='y', labelsize=12)
    plt.tight_layout()
    plt.savefig(f'/home/xusifan/LL/cal_cne6_902/photo_train/{prefix}feature_importance_split.png', dpi=300)
    plt.close()
    print(f'特征重要性（split）图已保存到 /home/xusifan/LL/cal_cne6_902/photo_train/{prefix}feature_importance_split.png')
...
def once_train_reg(train_df: pd.DataFrame, test_data_X):
    ...
    X_val_scaled = scaler.transform(X_val) # 验证集用训练集的参数转换
    # 修正：转回DataFrame并保留列名
    ...
    # ====== LightGBM gain/split重要性图 ======
    estimate(model, prefix='reg_')
...
def once_train_clf(train_df: pd.DataFrame, test_data_X):
    ...
    model.fit(
        ...
        eval_metric=['binary_logloss', 'auc', 'average_precision'],  # 评估指标
        callbacks=[
...
if __name__ == "__main__":
    ...
    result_dir = f'lightgbm_{train_window}_{delay}_{model_type}_260114'  # 用当前日期加以区分
    ...
    if train_window + delay > len(all_dates):
        ...
        print(f'训练窗口天数{train_window} 加上 预测未来天数{delay} 在已有数据范围（{all_dates[0]} - {all_dates[-1]}）内')
    ...
    # 每轮测试日期为 end_index 后 delay 个交易日
    test_index = end_index + delay

    # 每次循环：确定训练窗口和测试日期，准备训练和测试数据
    while end_index < len(all_dates)-delay:
        # 训练开始日期
        ...
        print(f"本次训练的测试日期: {test_date.strftime('%Y-%m-%d')}")
        ...
        if model_type == 'reg':
            ...
            # 回归模型评估
            # 只保留非NaN的部分
            mask = ~pd.isna(test_data_y_reg) & ~pd.isna(pred)
            y_true = test_data_y_reg[mask]
            y_pred = np.array(pred)[mask]
            rmse = np.sqrt(mean_squared_error(y_true, y_pred))  # 手动计算RMSE
