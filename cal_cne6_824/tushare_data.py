...
ts.set_token(token='fcc268ebc60170db2757a9d5ca830fb4a5a5b36b51ee13bd629ba982')  # Tushare 的 API key（密钥）
pro = ts.pro_api()  # 初始化 Tushare Pro 的接口对象，后续通过 pro.xxx() 方式获取数据
...
def get_trade_date(start_date=None, end_date=None, count=None, all_dates=None):
    ...
    """
    assert [start_date, end_date, count].count(None) <= 1  # 保证最多只有一个参数为 None，否则参数不合法
    trade_date = []
    # 3种情况
...
def pubDate_align_tradedate(df: pd.DataFrame, end_date, all_dates, pubDate_col='pubDate'):
    ...
    df.loc[valid_mask, 'aligned_date'] = align_date_vectorized(df.loc[valid_mask, pubDate_col])  # 对每个有效的公布日，找到其最近的交易日，赋值给对齐后的交易日aligned_date
    ...
    # 前向填充
    # 对每只股票，按时间排序后，对所有数据列做前向填充，保证每个交易日都能拿到最近一次公布的财报数据
...
def get_all_ts_codes(start_date, end_date):
    ...
    if ignore_exists and os.path.exists(path_in_date):  # 忽略重新下载/生成数据的步骤（ignore_exists = True），直接用已有的本地文件
        stock_basic_df_in_date = read_df(path_in_date)
    else:
        stock_basic_df = pro.stock_basic()
        stock_basic_df_in_date = stock_basic_df[(stock_basic_df['list_date'].astype(int)<=start_date)]  # 筛选出上市日期小于等于 start_date 的股票（即在起始日之前已上市的股票）
...
def get_bak_basic(all_ts_code, start_date, end_date):
    ...
    if ignore_exists and os.path.exists(path):
        ...
        '''
        用 industry_d 字典将三级行业归类到申万一级行业（L1）
        其中 industry_d 由 deepseek 自动归类生成，并非来自官方数据
        表格中：
        industry 列是 tushare 的 stock_basic() 接口直接获取到的 industry 行业因子
        L1 列是申万一级行业
        '''
        ins_table = {}
...
def get__by_ts_code(typ, ts_code):
    '''
    单独获取某只股票的某类财务报表（利润表、资产负债表、现金流量表），用于查漏补缺
    '''
    if typ==0:
        df = pro.income_vip(**{
            "ts_code": ts_code,
...
def get_balancesheet_by_season(periods, all_ts_codes):
    ...
    # get_balance()
    # 资产负债表
    # 批量获取指定季度（periods）所有A股上市公司（all_ts_codes）的资产负债表数据
...
def get_cashflow_by_season(periods, all_ts_codes):
    ...
    # get_cashflow()
    # 现金流量表
    # 批量获取指定季度（periods）所有A股上市公司（all_ts_codes）的现金流量表数据
...
def combine_finance(all_ts_codes, periods, income_df_list, balancesheet_df_list, cashflow_df_list):
    '''
    将季度利润表、资产负债表、现金流量表三类数据合并为一个完整的季度财务报表，并保存到本地 finance.csv
    '''
    print("合并财务报表")
    ...
    for index in range(len(df3)):
        ...
        with tqdm(df3[index].groupby('ts_code'), desc=f"合并财务报表{df3_paths[index]}: ") as pbar:
            for ts_code, g in pbar:
                pbar.set_description(f"{ts_code}")
                if g['end_date'].shape[0] != 25:  # 检查 end_date（报告期）数量是否等于25（即所有季度都齐全）
                    while 1:
                        try:
                            found_df = get__by_ts_code(index, ts_code)  # 如果不齐全，则调用 get__by_ts_code(index, ts_code) 单独补全缺失季度的数据
                            break
                        except Exception as ex:
                            pbar.set_description(f'{ex}。重试')
                            time.sleep(2)
    ...
    still_lacks.drop_duplicates(subset=['ts_code'], keep='last', ignore_index=True, inplace=True)  # 去重
...
def calc_pcf_ratio(all_ts_codes, all_dates, start_date, end_date, combined_df: pd.DataFrame, daily_basic_df_all: pd.DataFrame):
    '''计算每只股票的PCF（市现率）和多个TTM（滚动年）财务指标'''
    # TTM 代表过去连续 12 个月，但是在实际财务分析里，由于公司通常只按季度披露财报，所以一般用最近4个季度的财报数据之和来近似 TTM
    ...
    combined_df.loc[:, 'compr_inc_attr_p'] = combined_df['compr_inc_attr_p'].fillna(0)  # 缺失值填0
    ...
    need_ttm = [
        ...
        'net_profit',  #年内累加
        'cash_equivalent_increase',  #年内不累加，本身就是季度值
    ...
    def calculate_4_season_sum(group):
        ...
        for column in need_ttm:
            ...
            if column in ['cash_equivalent_increase']:
                # cash_equivalent_increase 年内不累加，直接取值
                group_filtered.loc[:, quarterly_column] = group_filtered[column]
            else:
                # 使用 .loc 进行安全赋值
                group_filtered.loc[:, quarterly_column] = np.where(
                    group_filtered['statDate'].dt.month == sum_month,  # 判断当前行的 statDate（报告期）是不是“季度末”
                    group_filtered[column],  # 如果是季度末，直接取本期累计值，作为本季度的值
                    group_filtered[column] - group_filtered[column].shift(1)  # 如果不是季度末，则用本期累计值减去上期累计值，得到本季度的值
                )
    ...
    # 计算 pcf_ratio = 当前市值除以过去12个月的现金盈利
